...
public class RecService {
	...
	public RecPageData searchList2(int reqPage, int branch, String keyword) throws SQLException {
		Connection conn = JDBCTemplate.getConnection();
		int numPerPage=10;
		int totalCount= new RecDao().searchCount2(conn,branch,keyword);
		int totalPage = (totalCount%numPerPage==0)?(totalCount/numPerPage):(totalCount/numPerPage)+1;
		int start = (reqPage-1)*numPerPage+1;
		int end = reqPage*numPerPage;
		ArrayList<Recruit> list= new RecDao().searchList2(conn,start,end,branch,keyword);
		for(int i = 0 ; i<list.size();i++) {
			list.get(i).setAmount2(list.get(i).getAmount());
			list.get(i).setAble2(list.get(i).getRecAble());
			list.get(i).setDate2(list.get(i).getRecDate());
